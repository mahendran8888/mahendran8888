...
df = breeze.get_historical_data(interval="1day",
                            from_date= "2024-2-1T07:00:00.000Z",
                            to_date= "2024-2-1T07:00:00.000Z",
                            stock_code="CNXBAN",
                            exchange_code="NSE",
                            product_type="cash")
...
for index, row in dfbnce1[1:].iterrows():
    ...
    if ((row['rsi_14'] < 30) and (previous_row['rsi_14'] > 30)) and index < 359:
        ...
        trade_log = trade_log.append({'Stockcode':row['stock_code'],'Right': row['right'],'strike_price':row['strike_price'],'Call':'Buy', 'Entry Time':row['datetime'],'Entry Price' : row['close']},ignore_index=True)
        # time.sleep(0.1)
