...
df = breeze.get_historical_data(interval="1day",
                            from_date= "2024-1-3T07:00:00.000Z",
                            to_date= "2024-1-3T07:00:00.000Z",
                            stock_code="CNXBAN",
                            exchange_code="NSE",
                            product_type="cash")
...
for index, row in df_bnce1[1:].iterrows():
    if index == 1:
        ...
        # print(trade_log['Entry Price'][0])
        ce_long_trade_triggered = 1
    # elif ce_long_trade_triggered == 1 and pd.to_numeric(trade_log['Entry Price'][0]) > 200:
    # elif ((pd.to_numeric(row['close']) < pd.to_numeric(trade_log['Entry Price'][0]) * 0.8) or (pd.to_numeric(row['close']) < ((pd.to_numeric(row['high']).max()*0.9))) or (index > 360)) and ce_long_trade_triggered == 1:
    elif (pd.to_numeric(row['close']) < ((pd.to_numeric(row['high']).max() * 0.5))) and ce_long_trade_triggered == 1:
        # print(row['close'])
...
for index, row in df_bnpe1[1:].iterrows():
    ...
    # elif ce_long_trade_triggered == 1 and pd.to_numeric(trade_log['Entry Price'][0]) > 200:
    # elif ((pd.to_numeric(row['close']) < pd.to_numeric(trade_log['Entry Price'][0]) * 0.8) or (pd.to_numeric(row['close']) < ((pd.to_numeric(row['high']).max()*0.9))) or (index > 360)) and pe_long_trade_triggered == 1:
    elif (pd.to_numeric(row['close']) < ((pd.to_numeric(row['high']).max() * 0.5))) and pe_long_trade_triggered == 1:
        # print(row['close'])
        trade_log = trade_log.append({'Exit Time': row['datetime'], 'Exit Price': row['close']}, ignore_index=True)
        pe_long_trade_triggered = 0
