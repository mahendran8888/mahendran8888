...
breeze = BreezeConnect(api_key="A7DZ04c81Vl77987w@687!62&4#y8B64")
breeze.generate_session(api_secret="`761360001007Y69P946T7701D2~2y@1", session_token="1585640")
...
print(sma1[["close","signal","signal price","datetime"]])
...
for index, row in sma[1:].iterrows():
    ...
    if (pd.to_numeric(previous_row['low']) < previous_row['sma']) & (pd.to_numeric(row['low']) > row['sma']):
...
#         take_profit = 0
#         # time.sleep(0.1)
    if (pd.to_numeric(previous_row['high']) > previous_row['sma']) & (pd.to_numeric(row['high']) < row['sma']):
