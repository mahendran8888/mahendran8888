...
df = breeze.get_historical_data(interval="1day",
                            from_date= "2024-1-29T07:00:00.000Z",
                            to_date= "2024-1-30T07:00:00.000Z",
                            stock_code="CNXBAN",
                            exchange_code="NSE",
                            product_type="cash")
...
df_bnce1['Entry'] = df_bnce1['close'][1]
df_bnpe1['Entry'] = df_bnpe1['close'][1]
df_bnce1['Stoplose'] = pd.to_numeric(df_bnce1['Entry'])*0.5
df_bnce1['Max'] = df_bnce1['high'].cummax()
df_bnce1['TP'] = pd.to_numeric(df_bnce1['Max'])*0.9
# df_bn_ce_pe =
# print(df_bnce1)
# print(df_bnce1)
...
for index, row in df_bnce1[1:].iterrows():
    ...
    elif ce_long_trade_triggered == 1 and (index == 360 or pd.to_numeric(row['close']) > (pd.to_numeric(df_bnce1['Entry'][0])+take_profit) or pd.to_numeric(row['close']) < (pd.to_numeric(df_bnce1['Entry'][0])-stop_lose)):
    ...
    if index == 1:
        trade_log = trade_log.append(
            {'Stockcode': row['stock_code'],'right': row['right'], 'strike_price': row['strike_price'], 'Call': 'Buy',
             'Entry Time': row['datetime'], 'Entry Price': row['close']}, ignore_index=True)
        ce_long_trade_triggered = 1
    elif ce_long_trade_triggered == 1 and (index == 360 or pd.to_numeric(row['close']) > (pd.to_numeric(df_bnpe1['Entry'][0])+take_profit) or pd.to_numeric(row['close']) < (pd.to_numeric(df_bnpe1['Entry'][0])-stop_lose)):
        trade_log = trade_log.append({'Exit Time': row['datetime'], 'Exit Price': row['close']}, ignore_index=True)
        ce_long_trade_triggered = 0
