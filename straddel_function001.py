...
def straddle001(d1,d2,tp,sl):
    ...
    for index, row in df_bnce1[1:].iterrows():
        ...
        elif ce_long_trade_triggered == 1 and (index == 360 or pd.to_numeric(row['close']) > (pd.to_numeric(df_bnce1['Entry'][0])+tp) or pd.to_numeric(row['close']) < (pd.to_numeric(df_bnce1['Entry'][0])-sl)):
        ...
        if index == 1:
            trade_log = trade_log.append(
                {'Stockcode': row['stock_code'],'right': row['right'], 'strike_price': row['strike_price'], 'Call': 'Buy',
                 'Entry Time': row['datetime'], 'Entry Price': row['close']}, ignore_index=True)
            ce_long_trade_triggered = 1
        elif ce_long_trade_triggered == 1 and (index == 360 or pd.to_numeric(row['close']) > (pd.to_numeric(df_bnpe1['Entry'][0])+tp) or pd.to_numeric(row['close']) < (pd.to_numeric(df_bnpe1['Entry'][0])-sl)):
            trade_log = trade_log.append({'Exit Time': row['datetime'], 'Exit Price': row['close']}, ignore_index=True)
            ce_long_trade_triggered = 0
    ...
    print(trade_log)
    # return trade_log
# straddle001(datetime.datetime(2024,1,25),datetime.datetime(2024,1,25))
