...
def rs10008():
    ...
    # if order detail have only one order, trigger sell order for buy order
        ...
        if len(order_detail) > 1:
            ...
            unique_order_id = pd.concat(
                [buy_order_detail['order_id'], sell_order_detail['user_remark']]).drop_duplicates(keep=False)
            ...
            if running_order_row.empty == False and float(running_order_price[0]) > (
                    float(running_order_avg_price[0]) * (100 + (p / 100))) and running_order_right == 'call':
                print(breeze.place_order(stock_code="CNXBAN",
                                         exchange_code="NFO",
                                         product="options",
                                         action="sell",
            ...
            if running_order_row.empty == False and float(running_order_price[0]) > (
                    float(running_order_avg_price[0]) * (100 + (p / 100))) and running_order_right == 'put':
                print(breeze.place_order(stock_code="CNXBAN",
                                         exchange_code="NFO",
                                         product="options",
                                         action="sell",
